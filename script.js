...
function setupActionBtn() {
    ...
    if (!block || document.getElementById('action-btn')) return;
    const isAdmin = (currentUser||'').toLowerCase() === 'adham';
    const btn = document.createElement('button');
    btn.id = 'action-btn';
    btn.className = 'action-btn';
    btn.innerHTML = isAdmin ? '👑 Admin Panel' : '✉️';
    btn.title = isAdmin ? 'Admin Panel' : 'Adminga xabar yuborish';
    btn.onclick = () => isAdmin ? switchScreen('dashboard-screen','admin-screen') : switchScreen('dashboard-screen','feedback-screen');
...
async function checkQRToken() {
    ...
    try {
        const r = await fetch(GOOGLE_SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ action: 'qr_register', token })
        });
        const result = await r.json();
        if (result.success) {
            // Avtomatik login
            localStorage.setItem('pro_exam_auth', 'true');
            localStorage.setItem('pro_exam_name', result.name);
            localStorage.removeItem('pro_exam_demo');
            isDemoUser = false;
            currentUser = result.name;
            const dnEl = document.getElementById('display-name');
            if (dnEl) dnEl.innerText = result.name;
            const snEl = document.getElementById('student-name');
            if (snEl) snEl.value = result.name;
            document.getElementById('global-nav').classList.remove('hidden');
            await loadUserStats(result.name);
            switchScreen('auth-screen', 'welcome-screen');
            setTimeout(() => {
                switchScreen('welcome-screen', 'dashboard-screen');
...
async function refreshAdminQR() {
    ...
    await dbSave('active_qr_token', { token: currentQRToken, created: new Date().toISOString() });
    const qrUrl = window.location.origin + window.location.pathname + '?qr_token=' + currentQRToken;
    // QR kod (Google Charts API bilan)
    const qrImgUrl = 'https://chart.googleapis.com/chart?chs=160x160&cht=qr&chl=' + encodeURIComponent(qrUrl) + '&choe=UTF-8';
    qrEl.innerHTML = `
        <img src="${qrImgUrl}" alt="QR" style="width:140px;height:140px;border-radius:12px;border:2px solid var(--glass-border);">
        <p style="font-size:0.72rem;color:var(--text-3);margin-top:8px;word-break:break-all;">${currentQRToken}</p>
        <button onclick="refreshAdminQR()" style="margin-top:8px;padding:8px 16px;border-radius:8px;background:var(--blue-soft);color:var(--primary);border:none;font-weight:700;font-size:0.82rem;cursor:pointer;">↺ Yangilash</button>`;
}
