...
async function downloadCertificate() {
    ...
    try {
        ...
        // FIX 1 (Simmetriya): Element o'lchamini scroll/clip ta'sirisiz aniq olish
        // scrollIntoView — elementni viewport markaziga olib keladi
        certEl.scrollIntoView({ block: 'center' });
        await new Promise(resolve => setTimeout(resolve, 80));

        // Aniq o'lcham: box-shadow va border kesilmasin uchun padding qo'shamiz
        const PADDING = 6; // px — o'ng ramka kesilishini oldini oladi
        const rect = certEl.getBoundingClientRect();
        const certWidth  = Math.round(rect.width)  + PADDING * 2;
        const certHeight = Math.round(rect.height) + PADDING * 2;
        ...
        const canvas = await html2canvas(certEl, {
            ...
            // FIX 2 (Mobil kadr): Koordinatalar — scroll va offset hisobga olinadi
            ...
            onclone: (doc) => {
                ...
                // FIX 2 (Mobil): Klon elementni scroll/transform dan ozod qilish
                ...
                if (clonedCert) {
                    ...
                    clonedCert.style.minHeight  = '608px';
                    clonedCert.style.overflow   = 'visible';
                    // FIX 1 (Simmetriya): Padding qo'shish — ramka kesilmasin
                    clonedCert.style.padding    = `${PADDING}px`;
                    clonedCert.style.boxSizing  = 'border-box';
                }

                // Sertifikat qatlamlari — backdrop-filter o'chirish
                ...
                if (container) {
                    ...
                    // FIX 1: overflow:visible — ramka qirqilmasin
                    container.style.overflow             = 'visible';
                    container.style.boxSizing            = 'border-box';
                }

                // css-cert-border — qattiq fon, border to'liq ko'rinsin
                ...
                // ::after pseudo-element html2canvas da ishlamaydi —
                // bino.png overlay o'rniga container foni yetarli
                const afterEl = doc.querySelector('.css-cert-container::after');
