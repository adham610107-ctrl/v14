    ...
    try {
        ...
        const canvas = await html2canvas(certEl, {
            ...
            onclone: (doc) => {
                ...
                const cert = doc.getElementById('printable-cert');
                if (cert) {
                    cert.style.width = '860px';
                    cert.style.height = 'auto';
                    cert.style.minHeight = '608px';
                }

                // Sertifikat qatlamlari — qattiq, premium fon
                const layers = [
                    '#printable-cert',
                    '.css-cert-container',
                    '.css-cert-border',
                ...
                layers.forEach(sel => {
                    ...
                    if (el) {
                        el.style.backdropFilter = 'none';
                        el.style.webkitBackdropFilter = 'none';
                        el.style.background = 'rgba(15, 15, 20, 0.95) !important';
                    }
                });

                // css-cert-container — bino.png orqa fon saqlansin
