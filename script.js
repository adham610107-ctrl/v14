...
async function authenticateUser() {
    ...
    // DEMO REJIM: keygen kiritilmagan
    if (!hasKeygen || keygenVal === '') {
        isDemoUser = true;
        localStorage.setItem('pro_exam_auth', 'true');
        localStorage.setItem('pro_exam_name', loginVal);
        localStorage.setItem('pro_exam_demo', 'true');
    ...
    try {
        ...
        const result = await r.json();
        if (result.success) {
            isDemoUser = result.isPaid === false; // server qaytargan paid statusga qarab
            localStorage.setItem('pro_exam_auth', 'true');
            ...
            try { sendLog('login', { login: loginVal, deviceId, isPaid: !isDemoUser }); } catch(e) {}
            switchScreen('auth-screen', 'welcome-screen');
            if (!isDemoUser) { startBlockCheck(); startHeartbeat(); }
        } else {
        ...
        modal.innerHTML = `
            <div style="background:var(--card-solid);border-radius:20px;padding:28px;max-width:340px;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
                ...
                <div style="display:flex;gap:10px;">
                    <button onclick="confirmAnim(false,this)" style="flex:1;padding:13px;border-radius:12px;background:var(--card-alt);border:1px solid var(--glass-border);color:var(--text);font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;">O'chirish</button>
                    <button onclick="confirmAnim(true,this)" style="flex:2;padding:13px;border-radius:12px;background:var(--primary);color:#fff;border:none;font-weight:800;cursor:pointer;font-family:'DM Sans',sans-serif;">Tushundim ✓</button>
                </div>
            </div>`;
        document.body.appendChild(modal);
        // Agar bekor qilsa checkbox ni qaytarish
        modal._checkbox = checkbox;
...
function confirmAnim(yes, btn) {
    const modal = btn.closest('[style*="position:fixed"]');
    const checkbox = modal._checkbox;
    if (yes) {
